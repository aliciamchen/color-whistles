...
def make_init_df(init_signals_dir=params.learning_sigs_dir):
    ...
    for referent_id, signal_id in params.init_signal_mappings.items():
        ...
        df_signal = preprocess.make_signal_df(
            ...
            referent=params.learning_referents[
                key_by_value(params.init_color_mappings, referent_id)
            ],
            speaker="init",
        )
...
def make_learn_df(df):
    ...
    for row in df.itertuples():
        ...
        df_signal = preprocess.make_signal_df(
            ...
            referent_id=params.init_color_mappings[row.referent_id],
...
def main(args):
    ...
    df_init.to_csv(os.path.join(output_dir, "init_signals_tidy.zip"), index=False)
    df_learn_long.to_csv(
        os.path.join(output_dir, "learn_signals_tidy.zip"), index=False
    )
    df_comm_long.to_csv(
        os.path.join(output_dir, "comm_signals_tidy.zip"), index=False
    )
